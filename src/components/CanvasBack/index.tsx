...
export default function CanvasBack(props: Props): JSX.Element {
  ...
  /** 初始化canvas **/
  const init = (row: number, col: number, width: number, height: number): void => {
  ...
  /** 绘制一帧 **/
  const drow = useCallback((dots: Dot[], row: number, col: number, ctx: CanvasRenderingContext2D, width: number, height: number): void => {
    ...
    for (let i = 0; i < row; i++) {
      for (let j = 0; j < col - 1; j++) {
        ...
        if (i > 0) {
          ...
          ctx.closePath();
          const c: number = Math.round((dots[k].color + dots[k1].color + dots[k3].color) / 3);
          ctx.fillStyle = `rgb(6, ${Math.round(c / 1.3)},${c})`;
          ctx.fill();
        }
      }
  ...
  const animate = (): void => {
    ...
    drow(data.current.dots, row, col, data.current.ctx as CanvasRenderingContext2D, width, height);
