...
import { Button, Table, Tooltip, Popconfirm, Modal, Form, Select, Input, InputNumber, message, Divider, Checkbox } from "antd";
...
function PowerAdmin() {
  ...
  // 新增&修改 模态框出现
  const onModalShow = (data: Power | null, type: OperateType) => {
    setModal({
      modalShow: true,
      nowData: data,
      operateType: type,
  ...
  const onOk = async () => {
    ...
    try {
      ...
      let rolepowers = new UpRolePowerReq();
      rolepowers.role_ids = selectRoles;
      if (modal.operateType === OperateType.Add) {
        const addres = (await MyFetch.post(ApiPath.AddPower, values)) as AddResp;
        rolepowers.power_id = addres.id;
        message.success("添加成功");
      } else {
        values.id = modal.nowData!.id;
        await MyFetch.post(ApiPath.UpPower, values);
        rolepowers.power_id = values.id;
        message.success("修改成功");
      }
      await MyFetch.post(ApiPath.UpRolePower, rolepowers);
      power_store.FetchAll(true);
  ...
  // 删除一条数据
  const onDel = async (record: Power) => {
    const params = { id: record.id };
    setLoading(true);
    await MyFetch.post(ApiPath.DelPower, params);
    power_store.FetchAll(true);
  ...
  // 构建表格字段
  const tableColumns: ColumnsType<TreeNodeType<Power>> = [
    ...
    },
    {
      ...
      render: (_v: number, record) => {
        const controls = [];
        userstore.HaveRight(PowerCode.PowerQuery) &&
          record.fake !== true &&
          controls.push(
        ...
        userstore.HaveRight(PowerCode.PowerUp) &&
          record.fake !== true &&
          controls.push(
        ...
        userstore.HaveRight(PowerCode.PowerDel) &&
          record.fake !== true &&
          controls.push(
  ...
  const table_tree = useMemo(() => {
    return GetPowerTree(power_store.items);
  }, [power_store.items]);
  // 构建表格数据
  const tableData = useMemo(() => {
    table_tree.datalist.forEach((item) => {
      (item as any as Power).code = item.data!.code;
      (item as any as Power).status = item.data!.status;
      (item as any as Power).desc = item.data!.desc;
    });
  ...
  return (
    <div>
      ...
      <Modal
        title={`${OperateName[modal.operateType]}权限->${modal.nowData?.title ?? ""}`}
      ...
      >
        <Form {...formItemLayout} form={form} initialValues={{ status: Status.Enable }}>
          ...
          >
            <Input placeholder="请输入权限名" disabled={formdisable} />
          </Form.Item>
          <Form.Item
            label="Code"
            name="code"
          ...
          >
            <Input placeholder="请输入权限Code" disabled={formdisable} />
          </Form.Item>
